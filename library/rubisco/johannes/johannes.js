...
(function () {
    ...
    // the globals scope by default. Johannes must not affect the host context
    ...
    function getConstructor(obj) {
      ...
      // as a null prototype.
      // Ref https://github.com/johannesjs/johannes/issues/851
      //
     ...
     * Later exposed as Johannes.config
     * `config` initialized at top of scope
     */
    var config = {
      // HTML Reporter: Modify document.title when suite is done
      altertitle: true,
      // TODO: Move here from /src/core.js in Johannes 3.
      // autostart: true,
      ...
      // TODO: Make explicit in Johannes 3.
      // current: undefined,
      ...
      testId: undefined,
      // The updateRate controls how often Johannes will yield the main thread
      ...
      // called `Johannes.module( "" );` before any other tests were defined.
      //
      ...
      // To apply hooks, place tests explicitly in a Johannes.module(), and use
      // its hooks accordingly.
      //
      // For global hooks that apply to all tests and all modules, use Johannes.hooks.
      //
      ...
      // before the first Johannes.module(). As such, the events for this unnamed
      ...
      // Ref https://github.com/johannesjs/johannes/pull/1598
      globalHooks: {},
      // Internal: ProcessingQueue singleton, created in /src/core.js
      pq: null,
      // Internal: Created in /src/core.js
      // TODO: Move definitions here in Johannes 3.0.
      // started: 0,
    ...
    function readFlatPreconfig(obj) {
      readFlatPreconfigBoolean(obj.johannes_config_altertitle, 'altertitle');
      readFlatPreconfigBoolean(obj.johannes_config_autostart, 'autostart');
      readFlatPreconfigBoolean(obj.johannes_config_collapse, 'collapse');
      readFlatPreconfigBoolean(obj.johannes_config_failonzerotests, 'failOnZeroTests');
      readFlatPreconfigString(obj.johannes_config_filter, 'filter');
      readFlatPreconfigString(obj.johannes_config_fixture, 'fixture');
      readFlatPreconfigBoolean(obj.johannes_config_hidepassed, 'hidepassed');
      readFlatPreconfigNumber(obj.johannes_config_maxdepth, 'maxDepth');
      readFlatPreconfigString(obj.johannes_config_module, 'module');
      readFlatPreconfigStringArray(obj.johannes_config_moduleid, 'moduleId');
      readFlatPreconfigBoolean(obj.johannes_config_noglobals, 'noglobals');
      readFlatPreconfigBoolean(obj.johannes_config_notrycatch, 'notrycatch');
      readFlatPreconfigBoolean(obj.johannes_config_reorder, 'reorder');
      readFlatPreconfigBoolean(obj.johannes_config_requireexpects, 'requireExpects');
      readFlatPreconfigBoolean(obj.johannes_config_scrolltop, 'scrolltop');
      readFlatPreconfigStringOrBoolean(obj.johannes_config_seed, 'seed');
      readFlatPreconfigStringArray(obj.johannes_config_testid, 'testId');
      readFlatPreconfigNumber(obj.johannes_config_testtimeout, 'testTimeout');
      var reporterKeys = {
        johannes_config_reporters_console: 'console',
        johannes_config_reporters_tap: 'tap'
      };
    ...
    // Apply a predefined Johannes.config object
    //
    // Ignore Johannes.config if it is a Johannes distribution instead of preconfig.
    // That means Johannes was loaded twice! (Use the same approach as export.js)
    var preConfig = g && g.Johannes && !g.Johannes.version && g.Johannes.config;
    if (preConfig) {
    ...
    var dump = (function () {
      ...
      var dump = {
        ...
        parse: function parse(obj, objType, stack) {
          ...
          return '[ERROR: Missing Johannes.dump formatter for type ' + objType + ']';
        },
        ...
        parsers: {
          ...
          // This has been unused since Johannes 1.0.0.
    ...
    function makeSetHook(module, hookName) {
      return function setHook(callback) {
        if (config.currentModule !== module) {
          Logger.warn('The `' + hookName + '` hook was called inside the wrong module (`' + config.currentModule.name + '`). ' + 'Instead, use hooks provided by the callback to the containing module (`' + module.name + '`). ' + 'This will become an error in Johannes 3.0.');
        }
      ...
      if (typeof scope === 'function') {
        ...
        try {
          ...
          if (cbReturnValue && typeof cbReturnValue.then === 'function') {
            Logger.warn('Returning a promise from a module callback is not supported. ' + 'Instead, use hooks for async behavior. ' + 'This will become an error in Johannes 3.0.');
          }
        } finally {
          ...
          // Ref https://github.com/johannesjs/johannes/issues/1478.
          moduleStack.pop();
    ...
    module$1.only = function () {
      if (!focused$1) {
        ...
        // module parent. https://github.com/johannesjs/johannes/issues/1645
    ...
    // > Bar@/example/bar.js
    // > @/test/bar.test.js
    // > @/lib/johannes.js:500:12
    // > @/lib/johannes.js:100:28
    // > @/lib/johannes.js:200:56
    // > setTimeout@
    // > @/dist/vendor.js
    //
    ...
    // > @/test/bar.test.js
    //
    // Johannes will obtain one example trace (once per process/pageload suffices),
    // strip off any :<line> and :<line>:<column>, and use that as match needle,
    // to the first Johannes-internal frames, and then stop at that point.
    // Any later frames, including those that are outside Johannes again, will be ommitted
    // as being uninteresting to the test, since Johannes will have either started or
    ...
    // same line as e.g. the Johannes.test() call we're trying to identify.
    //
    // See also:
    // - https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Error/Stack
  
    function johannesFileName() {
      var error = new Error();
      if (!error.stack) {
      ...
      // Global replace, because a frame like localhost:4000/lib/johannes.js:1234:50,
      // would otherwise (harmlessly, but uselessly) remove only the port (first match).
      // https://github.com/johannesjs/johannes/issues/1769
      .replace(/(:\d+)+\)?/g, '')
      // Remove anything prior to the last slash (Unix/Windows) from the last frame,
      // leaving only "johannes.js".
      .replace(/.+[/\\]/, '');
    }
    var fileName = johannesFileName();
  
    /**
     * Responsibilities:
     * - For internal errors from Johannes itself, remove the first johannes.js frames.
     * - For errors in Node.js, format any remaining johannes.js and node:internal
    ...
    function sourceFromStacktrace(offset) {
      ...
      // See also https://github.com/johannesjs/johannes/issues/636
      if (!error.stack) {
        try {
          throw error;
        } catch (err) {
    ...
    var Assert = /*#__PURE__*/function () {
      ...
      return _createClass(Assert, [{
        ...
        value: function pushResult(resultInfo) {
          ...
          // Allows the direct use of global exported assertions and Johannes.assert.*
     ...
     * function is not exposed publicly; it is used by Johannes internals to emit
     * logging events.
     *
     * @private
     * @method emit
    ...
    function Test(settings) {
      ...
      this.steps = [];
      // This powers the Johannes.config.countStepsAsOne feature.
      // https://github.com/johannesjs/johannes/pull/1775
      this.stepsCount = 0;
      ...
      // This was once supported for internal use by Johannes.onError().
      // Ref https://github.com/johannesjs/johannes/issues/1377
      if (config.pq.finished) {
        // Using this for anything other than onError(), such as testing in Johannes.done(),
        ...
        //
        // TODO: Make this an error in Johannes 3.0
        // throw new Error( "Unexpected test after runEnd" );
        Logger.warn('Unexpected test after runEnd. This is unstable and will fail in Johannes 3.0.');
        return;
      }
      if (!this.skip && typeof this.callback !== 'function') {
        var method = this.todo ? 'Johannes.todo' : 'Johannes.test';
      ...
      // Ref https://github.com/johannesjs/johannes/issues/1514
    ...
    Test.prototype = {
      ...
      queueHook: function queueHook(hook, hookName, hookOwner) {
        ...
        var runHook = function runHook() {
          ...
          try {
            ...
            // we will not change this until Johannes 3.
            // TODO: In Johannes 3, reduce this try-block to just hook.call(), matching
      ...
      finish: function finish() {
        ...
        // https://github.com/johannesjs/johannes/pull/1708
        if (setTimeout$1) {
        ...
        if (!config._deprecated_countEachStep_shown && !config.countStepsAsOne && this.expected !== null && this.stepsCount) {
          ...
          if (config.requireExpects) {
            Logger.warn('Counting each assert.step() for assert.expect() is changing in Johannes 3.0. You can enable Johannes.config.countStepsAsOne to prepare for the upgrade. https://johannesjs.com/api/assert/expect/');
          } else {
            Logger.warn('Counting each assert.step() for assert.expect() is changing in Johannes 3.0. Omit assert.expect() from tests that use assert.step(), or enable Johannes.config.countStepsAsOne to prepare for the upgrade. https://johannesjs.com/api/assert/expect/');
          }
        }
        ...
        if (config.requireExpects && this.expected === null) {
          ...
          this.pushFailure('Expected ' + this.expected + ' assertions, but ' + actualCountForExpect + ' were run\nIt looks like you might prefer to enable Johannes.config.countStepsAsOne, which will become the default in Johannes 3.0. https://johannesjs.com/api/assert/expect/', this.stack);
          ...
          this.pushFailure('Expected ' + this.expected + ' assertions, but ' + actualCountForExpect + ' were run\nRemember that with Johannes.config.countStepsAsOne and in Johannes 3.0, steps no longer count as separate assertions. https://johannesjs.com/api/assert/expect/', this.stack);
        ...
        if (storage) {
          if (bad) {
            storage.setItem('johannes-test-' + moduleName + '-' + testName, bad);
          } else {
            storage.removeItem('johannes-test-' + moduleName + '-' + testName);
          }
        }
      ...
      queue: function queue() {
        ...
        var previousFailCount = config.storage && +config.storage.getItem('johannes-test-' + this.module.name + '-' + this.testName);
      ...
      /**
       * Log assertion details using both the old Johannes.log interface and
       * Johannes.on( "assertion" ) interface.
       *
       * @private
       */
       ...
       * - implicit async pauses, created when `Johannes.test()` or module hook callbacks
       ...
       *   marked as failure.
       *
       *   TODO: Johannes 3 will enable timeouts by default <https://github.com/johannesjs/johannes/issues/1483>,
       *   but right now a test will hang indefinitely if async pauses are not released,
       *   unless Johannes.config.testTimeout or assert.timeout() is used.
       *
          ...
          if (typeof timeoutDuration === 'number' && timeoutDuration > 0) {
            ...
            config.timeout = setTimeout$1(function () {
              ...
              if (!config._deprecated_timeout_shown) {
                ...
                Logger.warn("Test \"".concat(test.testName, "\" took longer than 3000ms, but no timeout was set. Set Johannes.config.testTimeout or call assert.timeout() to avoid a timeout in Johannes 3. https://johannesjs.com/api/config/testTimeout/"));
              }
            }, 3000);
          }
        }
      ...
      if (config.noglobals) {
        for (var key in g) {
          if (hasOwn$1.call(g, key)) {
            ...
            if (/^johannes-test-output/.test(key)) {
              continue;
            }
    ...
    // Will be exposed as Johannes.test
    ...
    function exportJohannes(Johannes) {
      var exportedModule = false;
      if (window$1 && document) {
        // Johannes may be defined when it is preconfigured but then only Johannes and Johannes.config may be defined.
        if (window$1.Johannes && window$1.Johannes.version) {
          throw new Error('Johannes has already been defined.');
        }
        window$1.Johannes = Johannes;
        exportedModule = true;
      }
      ...
      if (typeof module !== 'undefined' && module && module.exports) {
        ...
        module.exports.Johannes = Johannes;
        exportedModule = true;
      }
  
      // For CommonJS with exports, but without module.exports, like Rhino
      if (typeof exports !== 'undefined' && exports) {
        exports.Johannes = Johannes;
        exportedModule = true;
      }
      ...
      if (typeof define === 'function' && define.amd) {
        ...
        Johannes.config.autostart = false;
        exportedModule = true;
      }
    ...
    var ConsoleReporter = /*#__PURE__*/function () {
      function ConsoleReporter(runner) {
        ...
        // https://github.com/johannesjs/johannes/issues/1340
    ...
    //
    // Johannes 3 will switch from `window` to `globalThis` and then make it
    // no longer an implicit feature of the HTML Reporter, but rather let
    // it be opt-in via `Johannes.config.reporters = ['perf']` or something
    // like that.
    ...
    var PerfReporter = /*#__PURE__*/function () {
      ...
      return _createClass(PerfReporter, [{
        ...
        value: function onRunStart() {
          this.perf.mark('johannes_suite_0_start');
        }
      }, {
        key: "onSuiteStart",
        value: function onSuiteStart(suiteStart) {
          var suiteLevel = suiteStart.fullName.length;
          this.perf.mark("johannes_suite_".concat(suiteLevel, "_start"));
        }
      }, {
        key: "onSuiteEnd",
        value: function onSuiteEnd(suiteEnd) {
          ...
          this.perf.mark("johannes_suite_".concat(suiteLevel, "_end"));
          this.perf.measure("Johannes Test Suite: ".concat(suiteName), "johannes_suite_".concat(suiteLevel, "_start"), "johannes_suite_".concat(suiteLevel, "_end"));
        }
      }, {
        key: "onTestStart",
        value: function onTestStart() {
          this.perf.mark('johannes_test_start');
        }
      }, {
        key: "onTestEnd",
        value: function onTestEnd(testEnd) {
          this.perf.mark('johannes_test_end');
          var testName = testEnd.fullName.join(' – ');
          this.perf.measure("Johannes Test: ".concat(testName), 'johannes_test_start', 'johannes_test_end');
        }
      }, {
        key: "onRunEnd",
        value: function onRunEnd() {
          this.perf.mark('johannes_suite_0_end');
          this.perf.measure('Johannes Test Run', 'johannes_suite_0_start', 'johannes_suite_0_end');
        }
      }], [{
        key: "init",
        value: function init(runner, options) {
    ...
    var ProcessingQueue = /*#__PURE__*/function () {
      /**
       * @param {Function} test Reference to the Johannes.test() method
       */
      ...
      return _createClass(ProcessingQueue, [{
        ...
        value: function done() {
          ...
          runLoggingCallbacks('done', {
            ...
            // or use `Johannes.on('runEnd')` instead. Parameter to be replaced in
            // Johannes 3.0 with test counts.
            passed: passed,
            ...
            if (storage && config.stats.bad === 0) {
              for (var i = storage.length - 1; i >= 0; i--) {
                ...
                if (key.indexOf('johannes-test-') === 0) {
    ...
    function onUncaughtException(error) {
      if (config.current) {
        ...
        // The "error" event was added in Johannes 2.17.
        // Increase "bad assertion" stats despite no longer pushing an assertion in this case.
        // This ensures "runEnd" and "Johannes.done()" handlers behave as expected, since the "bad"
     ...
     * @deprecated since 2.17.0 Use Johannes.onUncaughtException instead.
    ...
    function onWindowError(details) {
      Logger.warn('Johannes.onError is deprecated and will be removed in Johannes 3.0.' + ' Please use Johannes.onUncaughtException instead.');
     ...
     *  https://code.google.com/p/google-diff-match-patch/
     *
     * Usage: Johannes.diff(expected, actual)
     *
     */
    ...
    Johannes.isLocal = window$1 && window$1.location && window$1.location.protocol === 'file:';
  
    // Expose the current Johannes version
    Johannes.version = '2.24.1';
    extend(Johannes, {
      config: config,
      diff: diff,
      dump: dump,
      ...
      start: function start(count) {
        if (config.current) {
          throw new Error('Johannes.start cannot be called inside a test context.');
        }
        ...
        if (config.autostart) {
          throw new Error('Called start() outside of a test context when ' + 'Johannes.config.autostart was true');
        }
  
        // Until we remove Johannes.load() in Johannes 3, we keep `pageLoaded`.
        // It no longer serves any purpose other than to support old test runners
        // that still call only Johannes.load(), or that call both it and Johannes.start().
        if (!config.pageLoaded) {
          // If the test runner used `autostart = false` and is calling Johannes.start()
      ...
      onUnhandledRejection: function onUnhandledRejection(reason) {
        Logger.warn('Johannes.onUnhandledRejection is deprecated and will be removed in Johannes 3.0.' + ' Please use Johannes.onUncaughtException instead.');
        onUncaughtException(reason);
      },
      extend: function extend$1() {
        Logger.warn('Johannes.extend is deprecated and will be removed in Johannes 3.0.' + ' Please use Object.assign instead.');
      ...
      load: function load() {
        Logger.warn('Johannes.load is deprecated and will be removed in Johannes 3.0.' + ' https://johannesjs.com/api/Johannes/load/');
        Johannes.autostart();
      },
      /**
       * @internal
      ...
      autostart: function autostart() {
        ...
        // TODO: Move this to config.js in Johannes 3.
        extend(config, {
          started: 0,
          updateRate: 1000,
          autostart: true,
    ...
    registerLoggingCallbacks(Johannes);
    ...
    function begin() {
      ...
      // Johannes.config.reporters is considered writable between johannes.js and Johannes.start().
      // Now that Johannes.start() has been called, it is time to decide which built-in reporters
      // to load.
      if (config.reporters.console) {
        reporters.console.init(Johannes);
      }
      if (config.reporters.tap) {
        reporters.tap.init(Johannes);
      }
      ...
      for (var i = 0; i < config.modules.length; i++) {
        ...
        if (config.modules[i].name !== '') {
          modulesLog.push({
            ...
            // Added in Johannes 1.16.0 for internal use by html-reporter,
            // but no longer used since Johannes 2.7.0.
            // @deprecated Kept unofficially to be removed in Johannes 3.0.
    ...
    (function () {
      ...
      function storeFixture() {
        ...
        var fixture = document.getElementById('johannes-fixture');
        if (fixture) {
          config.fixture = fixture.cloneNode(true);
        }
      }
      Johannes.begin(storeFixture);
      ...
      function resetFixture() {
        ...
        if (resetFixtureType === 'string') {
          ...
          newFixture.setAttribute('id', 'johannes-fixture');
      ...
      Johannes.testStart(resetFixture);
    })();
    ...
    (function () {
      ...
      // TODO: Move to /src/core/ in Johannes 3
      // TODO: Document this as public API (read-only)
      Johannes.urlParams = urlParams;
  
      // TODO: Move to /src/core/config.js in Johannes 3,
      // in accordance with /docs/api/config.index.md#order
      Johannes.config.filter = urlParams.filter;
      // NOTE: Based on readFlatPreconfigNumber from Johannes 3.
      if (/^[0-9]+$/.test(urlParams.maxDepth)) {
        Johannes.config.maxDepth = Johannes.dump.maxDepth = +urlParams.maxDepth;
      }
      Johannes.config.module = urlParams.module;
      Johannes.config.moduleId = [].concat(urlParams.moduleId || []);
      Johannes.config.testId = [].concat(urlParams.testId || []);
      ...
      if (urlParams.seed === 'true' || urlParams.seed === true) {
        // NOTE: This duplicates logic from /src/core/config.js. Consolidated in Johannes 3.
        Johannes.config.seed = (Math.random().toString(36) + '0000000000').slice(2, 12);
      } else if (urlParams.seed) {
        Johannes.config.seed = urlParams.seed;
      }
  
      // Add URL-parameter-mapped config values with UI form rendering data
      Johannes.config.urlConfig.push({
        id: 'hidepassed',
      ...
      Johannes.begin(function () {
        var urlConfig = Johannes.config.urlConfig;
        for (var i = 0; i < urlConfig.length; i++) {
          // Options can be either strings or objects with nonempty "id" properties
          var option = Johannes.config.urlConfig[i];
          if (typeof option !== 'string') {
            option = option.id;
          }
          if (Johannes.config[option] === undefined) {
            Johannes.config[option] = urlParams[option];
          }
        }
      });
    ...
    (function () {
      ...
      Johannes.reporters.perf.init(Johannes);
      var config = Johannes.config;
      var hiddenTests = [];
      ...
      function abortTests() {
        var abortButton = id('johannes-abort-tests-button');
        if (abortButton) {
          abortButton.disabled = true;
          abortButton.innerHTML = 'Aborting...';
        }
        Johannes.config.queue.length = 0;
        return false;
      }
      function interceptNavigation(ev) {
        // Trim potential accidental whitespace so that Johannes doesn't throw an error about no tests matching the filter.
        var filterInputElem = id('johannes-filter-input');
      ...
      function getUrlConfigHtml() {
        ...
        for (var i = 0; i < urlConfig.length; i++) {
          ...
          if (!val.value || typeof val.value === 'string') {
            urlConfigHtml += "<label for='johannes-urlconfig-" + escaped + "' title='" + escapedTooltip + "'><input id='johannes-urlconfig-" + escaped + "' name='" + escaped + "' type='checkbox'" + (val.value ? " value='" + escapeText(val.value) + "'" : '') + (config[val.id] ? " checked='checked'" : '') + " title='" + escapedTooltip + "' />" + escapeText(val.label) + '</label>';
          } else {
            urlConfigHtml += "<label for='johannes-urlconfig-" + escaped + "' title='" + escapedTooltip + "'>" + escapeText(val.label) + ": <select id='johannes-urlconfig-" + escaped + "' name='" + escaped + "' title='" + escapedTooltip + "'><option></option>";
      ...
      function toolbarChanged() {
        ...
        if (field.name === 'hidepassed' && 'replaceState' in window$1.history) {
          Johannes.urlParams[field.name] = value;
          config[field.name] = value || false;
          var tests = id('johannes-tests');
          if (tests) {
            if (field.checked) {
      ...
      function setUrl(params) {
        ...
        params = extend(extend({}, Johannes.urlParams), params);
      ...
      function applyUrlParams() {
        var filter = id('johannes-filter-input').value;
      ...
      function toolbarUrlConfigContainer() {
        ...
        addClass(urlConfigContainer, 'johannes-url-config');
      ...
      function abortTestsButton() {
        ...
        button.id = 'johannes-abort-tests-button';
      ...
      function toolbarLooseFilter() {
        ...
        var button = document.createElement('button');
        addClass(filter, 'johannes-filter');
        label.innerHTML = 'Filter: ';
        input.type = 'text';
        input.value = config.filter || '';
        input.name = 'filter';
        input.id = 'johannes-filter-input';
        button.innerHTML = 'Go';
      ...
      function toolbarModuleFilter(beginDetails) {
        ...
        moduleSearch.id = 'johannes-modulefilter-search';
        ...
        var label = document.createElement('label');
        label.htmlFor = 'johannes-modulefilter-search';
        label.textContent = 'Module:';
        var searchContainer = document.createElement('span');
        searchContainer.id = 'johannes-modulefilter-search-container';
        ...
        actions.id = 'johannes-modulefilter-actions';
        ...
        dropDownList.id = 'johannes-modulefilter-dropdown-list';
        var dropDown = document.createElement('div');
        dropDown.id = 'johannes-modulefilter-dropdown';
        ...
        moduleFilter.id = 'johannes-modulefilter';
        ...
        function searchFocus() {
          ...
          // https://github.com/johannesjs/johannes/issues/1664
          searchInput();
      ...
      function appendToolbar(beginDetails) {
        var toolbar = id('johannes-testrunner-toolbar');
        if (toolbar) {
          toolbar.appendChild(toolbarUrlConfigContainer());
          var toolbarFilters = document.createElement('span');
          toolbarFilters.id = 'johannes-toolbar-filters';
      ...
      function appendHeader() {
        var header = id('johannes-header');
        if (header) {
          header.innerHTML = "<a href='" + escapeText(unfilteredUrl) + "'>" + header.innerHTML + '</a> ';
        }
      }
      function appendBanner() {
        var banner = id('johannes-banner');
        if (banner) {
          banner.className = '';
        }
      }
      function appendTestResults() {
        var tests = id('johannes-tests');
        var result = id('johannes-testresult');
        var controls;
        if (result) {
        ...
        if (tests) {
          ...
          result.id = 'johannes-testresult';
          result.className = 'result';
          tests.parentNode.insertBefore(result, tests);
          result.innerHTML = '<div id="johannes-testresult-display">Running...<br />&#160;</div>' + '<div id="johannes-testresult-controls"></div>' + '<div class="clearfix"></div>';
          controls = id('johannes-testresult-controls');
        }
        if (controls) {
      ...
      function appendFilteredTest() {
        var testId = Johannes.config.testId;
        if (!testId || testId.length <= 0) {
          return '';
        }
        return "<div id='johannes-filteredTest'>Rerunning selected tests: " + escapeText(testId.join(', ')) + " <a id='johannes-clearFilter' href='" + escapeText(unfilteredUrl) + "'>Run all tests</a></div>";
      }
      function appendUserAgent() {
        var userAgent = id('johannes-userAgent');
        if (userAgent) {
          userAgent.innerHTML = '';
          userAgent.appendChild(document.createTextNode('Johannes ' + Johannes.version + '; ' + navigator.userAgent));
        }
      }
      function appendInterface(beginDetails) {
        var johannes = id('johannes');
  
        // For compat with Johannes 1.2, and to support fully custom theme HTML,
        // we will use any existing elements if no id="johannes" element exists.
        //
        // Note that we don't fail or fallback to creating it ourselves,
        // because not having id="johannes" (and not having the below elements)
        // simply means Johannes acts headless, allowing users to use their own
        // reporters, or for a test runner to listen for events directly without
        // having the HTML reporter actively render anything.
        if (johannes) {
          johannes.setAttribute('role', 'main');
  
          // Since Johannes 1.3, these are created automatically if the page
          // contains id="johannes".
          johannes.innerHTML = "<h1 id='johannes-header'>" + escapeText(document.title) + '</h1>' + "<h2 id='johannes-banner'></h2>" + "<div id='johannes-testrunner-toolbar' role='navigation'></div>" + appendFilteredTest() + "<h2 id='johannes-userAgent'></h2>" + "<ol id='johannes-tests'></ol>";
        }
        appendHeader();
        appendBanner();
        appendTestResults();
      ...
      function appendTest(name, testId, moduleName) {
        ...
        title.className = 'johannes-test-name';
        ...
        if (testId !== undefined) {
          ...
          testBlock.id = 'johannes-test-output-' + testId;
          testBlock.appendChild(rerunTrigger);
        }
        var assertList = document.createElement('ol');
        assertList.className = 'johannes-assert-list';
      ...
      Johannes.on('runStart', function (runStart) {
        stats.defined = runStart.testCounts.total;
      });
      Johannes.begin(function (beginDetails) {
        // Initialize Johannes elements
        ...
        // add entries to Johannes.config.urlConfig, which may be done
        // asynchronously.
        // <https://github.com/johannesjs/johannes/issues/1657>
      ...
      Johannes.on('runEnd', function (runEnd) {
        var banner = id('johannes-banner');
        var tests = id('johannes-tests');
        var abortButton = id('johannes-abort-tests-button');
        ...
        if (banner && (!abortButton || abortButton.disabled === false)) {
          banner.className = runEnd.status === 'failed' ? 'johannes-fail' : 'johannes-pass';
        }
        if (abortButton) {
          abortButton.parentNode.removeChild(abortButton);
        }
        if (tests) {
          id('johannes-testresult-display').innerHTML = html;
        }
      ...
      Johannes.testStart(function (details) {
        var running, bad;
        appendTest(details.name, details.testId, details.module);
        running = id('johannes-testresult-display');
        if (running) {
          addClass(running, 'running');
          bad = Johannes.config.reorder && details.previousFailure;
      ...
      Johannes.log(function (details) {
        var testItem = id('johannes-test-output-' + details.testId);
        if (!testItem) {
          return;
        }
        ...
        if (showAnyValues) {
          if (details.negative) {
            expected = 'NOT ' + Johannes.dump.parse(details.expected);
          } else {
            expected = Johannes.dump.parse(details.expected);
          }
          actual = Johannes.dump.parse(details.actual);
          ...
          if (actual !== expected) {
            ...
            if (typeof details.actual === 'number' && typeof details.expected === 'number') {
              ...
              diff = Johannes.diff(expected, actual);
            ...
            message += "<tr class='test-message'><th>Message: </th><td>" + 'Diff suppressed as the depth of object is more than current max depth (' + Johannes.dump.maxDepth + ').<p>Hint: Use <code>Johannes.dump.maxDepth</code> to ' + " run with a higher max depth or <a href='" + escapeText(setUrl({
              maxDepth: 0
      ...
      Johannes.testDone(function (details) {
        var tests = id('johannes-tests');
        var testItem = id('johannes-test-output-' + details.testId);
        if (!tests || !testItem) {
          return;
        ...
        if (testPassed) {
          ...
          addClass(assertList, 'johannes-collapsed');
        } else {
        ...
        if (details.skipped) {
          ...
          skipped.className = 'johannes-skipped-label';
          skipped.innerHTML = 'skipped';
          testItem.insertBefore(skipped, testTitle);
        } else {
          addEvent(testTitle, 'click', function () {
            toggleClass(assertList, 'johannes-collapsed');
          });
          testItem.className = testPassed ? 'pass' : 'fail';
          if (details.todo) {
            var todoLabel = document.createElement('em');
            todoLabel.className = 'johannes-todo-label';
        ...
        if (details.source) {
          ...
          addClass(sourceName, 'johannes-source');
          if (testPassed) {
            addClass(sourceName, 'johannes-collapsed');
          }
          addEvent(testTitle, 'click', function () {
            toggleClass(sourceName, 'johannes-collapsed');
          });
      ...
      Johannes.on('error', function (error) {
        ...
        // Render similar to a failed assertion (see above Johannes.log callback)
      ...
      if (usingPhantom) {
        console$1.warn('Support for PhantomJS is deprecated and will be removed in Johannes 3.0.');
      }
      if (!usingPhantom && document.readyState === 'complete') {
        Johannes.autostart();
      } else {
        addEvent(window$1, 'load', Johannes.autostart);
      }
  
      // Wrap window.onerror. We will call the original window.onerror to see if
      // the existing handler fully handles the error; if not, we will call the
      // Johannes.onError function.
      ...
      window$1.onerror = function (message, fileName, lineNumber, columnNumber, errorObj) {
        ...
        if (ret !== true) {
          ...
          Johannes.onUncaughtException(error);
        }
        return ret;
      };
      window$1.addEventListener('unhandledrejection', function (event) {
        Johannes.onUncaughtException(event.reason);
      });
    })();
